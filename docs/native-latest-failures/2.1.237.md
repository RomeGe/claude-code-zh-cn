# macOS native latest candidate failure: 2.1.237
...
```bash
node scripts/verify-upstream-compat.js --baseline 2.1.237 --skip-latest --native-macos-arm64 --json
node scripts/generate-upstream-text-diff.js --to 2.1.237 --native-macos-arm64
