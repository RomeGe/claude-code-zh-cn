# macOS native latest candidate failure: 2.1.246
...
```bash
node scripts/verify-upstream-compat.js --baseline 2.1.246 --skip-latest --native-macos-arm64 --json
node scripts/generate-upstream-text-diff.js --to 2.1.246 --native-macos-arm64
