# macOS native latest candidate failure: 2.1.270
...
```bash
node scripts/verify-upstream-compat.js --baseline 2.1.270 --skip-latest --native-macos-arm64 --json
node scripts/generate-upstream-text-diff.js --to 2.1.270 --native-macos-arm64
